...
- <span class="blue">Second, use the `slice` function to create two dataframes: one called `training` consisting of the `training_rows`, and another called `test` consisting of the remaining rows of `arm_span`.</span>
...
- When we split data, we're randomly separating our observations into _training_ and _test_ sets.
...
_Whenever you split data into training and test, always use `set.seed` first._
...
- <span class="blue">Fit a line of best fit model to our `training` data and assign it the name `best_training`.</span>
