...
response_file_names <- gsub("(^lab[1-9][a-z]).Rpres", "\\1_response\\2.Rmd", lab_file_names) 
...
for (i in seq_along(lab_file_locations)) {
  ...
  questions <- gsub("\\*\\*", "", questions)
  questions <- gsub("`r nrow\\(cdc\\)`", "15,624", questions)
  
  questions <- paste(questions, 
                     collapse = "\n\n\\newline\n\n\\newline\n\n\\newline\n\n")
  response_dest_file <- paste0("Lab Response Sheets/Rmd/", 
                               response_file_names[i])
  writeLines(c(header, questions), response_dest_file)
  ...
  rmarkdown::render(response_dest_file, output_dir = "Lab Response Sheets/docx/")
}
