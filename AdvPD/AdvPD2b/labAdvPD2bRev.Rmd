...
Use the following command to load the `cdc` data into <b>RStudio</b>:  

```{r, eval = FALSE, echo = TRUE}
data(cdc)
```

Use the following command to open the spreadsheet containing the data:  
...
**Use the boxplot and summary statistics and answer the statistical questions.**
...
Transcribe the code into a sentence. That is, imagine the computer was sentinent and could understand verbal directions.  
...
```{r, eval = FALSE, echo = TRUE}
cdc_old <- filter(cdc, age != "12 years old or younger", age != "13 years old")
```
