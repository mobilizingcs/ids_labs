...
New functions which makes difficult programming tasks, such as creating plots, easier can also be developed and shared via packages.  

### <b>Installing and loading packages</b>  

To install a new packages (Normally only performed once) use the command below:

```{r, eval = FALSE,echo=TRUE}
install.packages("package_name")
```

After installing a package, the new functions can be loaded into **R** for use by using the `library()` function
...
Packages should be loaded with `library()` each time you restart **R**.  

## Packages Needed for Text Analysis

To help us begin to analyze words/text, install the `tidytext` package.  
...
Once your installation is complete, click on the _Packages_ pane in RStudio.  

**How many packages were added to your _User Library_?**  

Now that `tidytext` has (hopefully) been installed successfully, load the functions contained in the package.
...
**What happens when you load the `wordcloud` package?**
