...
- <span class="blue">Use the `leaflet()` function and the `mtns` data to create the `leaf` that we can use for mapping.</span>

```{r, eval = FALSE, echo = TRUE}
mtns_leaf <- leaflet(____)
```

- <span class="blue">Then, insert `mtns_leaf` into the `addTiles()` function and assign the output the name `mtns_map`.</span>
...
- <span class="blue">Fill in the blanks below with the basic map we've created and the values for latitude and longitude.</span>
    
```{r, eval = FALSE, echo = TRUE}
addMarkers(map = ____, lng = ~____, lat = ~____)
```

- <span class="blue">Supply the `peak` variable, in a similar way as we supplied the `lat` and `long` variables, to the `popup` argument and include it in the code above.</span>
...
- <span class="blue">Fill in the blanks below to create a new variable that assigns a color to each mountain based on the `state` it's located in.</span>
```{r, eval = FALSE, echo = TRUE}
...
- Now that we've added a new variable, we need to re-build `mtns_leaf` and `mtns_map` to use it.
    - <span class="blue">Create `mtns_leaf` and `mtns_map` as you did before.</span>
...
- It's hard to know just what the different colors mean so let's add a legend.
  ...
  - <span class="blue">Then, fill in the blanks below to place a legend in the top-right hand corner.</span>
```{r, eval = FALSE, echo = TRUE}
