...
```{r, eval = FALSE}
# all rows satisfy both conditions
df <- cdc %>%
  filter(height > 1.8, weight > 90)
# taller than 1.8 meters AND weighs more than 90 kilograms
```

<li>If you want to subset by one condition OR another condition, use the vertical bar `|`.</li>

```{r}
# all rows satisfy one of the following conditions
df <- cdc %>%
...
cdc_complete %>%
  summarize(mean(height), mean(weight), sd(height), median(weight))
```
