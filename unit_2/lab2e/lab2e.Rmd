...
- <span class="blue">To begin, use the `data` function to load the `slasher` data file.</span>
...
- <span class="blue">Ten simulations is not enough. Use `do`, `tally` and `shuffle` functions to `shuffle` the `survival` variable and `tally` the percentage of women who survived 500 times. `Assign` your 500 shuffles the name `shuffled_survivors`. Use `set.seed(1)`. </span>
...
- To compute the differences for each shuffle we can use the `mutate` function.
    - <span class="blue">Fill in the blanks to add a new column that contains the difference between `Survives.Female` and `Survives.Male` to our `shuffled_survivors` data.</span>
...
- <span class="blue">Create a `histogram` of the `difference`s in our `shuffled_survivors` data. Based on your plot, answer the following:</span>
...
- **Does shuffling the `gender` variable instead of the `survival` variable change your answer to the question?**
