...
- Make a `dotPlot` of your _predominant_ color's scores. Make sure to include the `nint` option.
...
```{r, eval = FALSE, echo = TRUE}
mm_diff <- function(variable, my_data) {
  mean_val <- mean(variable, data = my_data)
  med_val <- median(variable, data = my_data)
  abs(mean_val - med_val)
}
```

- The function takes two _generic_ arguments: `variable` and `my_data`
...
- Using the previous example as a guide, create a function called `Range` (_Note the capital 'R'_) that calculates the _range_ of a variable by filling in the blanks below:
