...
- Run this code 10 times and compute the _proportion_ of `"rap"` songs you drew from the 10.
  - Vocabulary Check: A _proportion_ is a fraction of the whole.
...
- Instead of running the same line of code multiple times ourselves we can use `R` to `do()` multiple repetitions for us.
    - Fill in the blanks below to `do` the `sample` code from the previous slide _50_ times:
```{r, eval = FALSE, echo = TRUE}
do(___) * sample(___, ___ = ___, ___ = ___)
```
- Recall that we need to store our results to be able to perform analysis.
- _Assign_ the 50 selected songs the name `draws` and then `View` your file.
